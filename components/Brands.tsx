...
const Brands: React.FC = () => {
    return (
        <section id="brands" className="py-16 sm:py-20 bg-gray-50" aria-labelledby="brands-heading">
            <div className="container mx-auto px-4 sm:px-6 text-center">
                <AnimatedSection animation="fade-in-up">
                    <h2 id="brands-heading" className="text-sm font-bold uppercase text-firetech-red tracking-widest mb-2">Our Partners</h2>
                        ...
                        <ul className="flex items-center justify-center md:justify-start [&_li]:mx-4 md:[&_li]:mx-8 [&_img]:max-w-none animate-infinite-scroll">
                            {brandLogos.map((brand, index) => (
                                <li key={index}>
                                    <img 
                                      src={brand.logoUrl} 
                                      alt={`${brand.name} - Trusted fire safety and construction partner`} 
                                      className="h-10 md:h-12 w-auto object-contain filter grayscale hover:grayscale-0 transition-all duration-300 cursor-pointer hover:scale-110" 
                                      loading="lazy"
                                      width="120"
                                      height="48"
                                    />
